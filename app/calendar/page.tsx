...
import { getCalendarEvents, createCalendarEvent, updateCalendarEvent, deleteCalendarEvent } from "@/app/actions/calendar";
  ...
  return (
    <div className="min-h-screen bg-green-50">
                    ...
                    <div className="space-y-4">
                      {events
                        ...
                        .map((event) => (
                          ...
                          >
                            <div className="h-12 w-12 rounded-xl bg-primary/10 flex items-center justify-center">{getEventTypeIcon(event.type)}</div>
                            <div className="flex-1 min-w-0">
                              <h4 className="font-semibold text-gray-900 text-lg">{event.title}</h4>
                              <p className="text-gray-600 truncate">
                                {new Date(event.date + "T00:00:00").toLocaleDateString()} • {event.time} {event.timezone && `(${event.timezone})`} •{" "}
                                {event.location}
                              </p>
                            </div>
                            <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex items-center gap-4 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                              <CardTitle className="text-xl font-semibold text-gray-900 font-serif truncate">{event.title}</CardTitle>
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-700">{new Date(event.date + "T00:00:00").toLocaleDateString()}</span>
                          </div>
                          <div className="flex items-center gap-2">
            ...
            >
              ...
              <Card className="border-gray-100 shadow-sm">
                <CardHeader>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <CardTitle className="text-2xl font-semibold text-gray-900 font-serif">Vacation Schedule</CardTitle>
                      <CardDescription className="text-gray-600">Team member time off and vacation requests</CardDescription>
                    </div>
                    {canEditCalendar && (
                      <Button
                        size="sm"
                      ...
                      {vacations.map((vacation) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              <Badge className={`${getVacationStatusColor(vacation.status)} flex-shrink-0`}>{vacation.status}</Badge>
      ...
      >
        <DialogContent className="sm:max-w-[600px]">
          ...
          <DialogFooter>
            ...
            {selectedEvent?.onlineMeeting && <Button onClick={() => handleJoinEvent(selectedEvent)}>Join Meeting</Button>}
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-semibold font-serif">{isEditingVacation ? "Edit Vacation Request" : "Add New Vacation"}</DialogTitle>
            <DialogDescription>{isEditingVacation ? "Update vacation request details" : "Create a new vacation request for a team member"}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-6 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label
